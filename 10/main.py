...
Joltages = tuple[int, ...]
Machine = tuple[Indicators, tuple[Button, ...], Joltages]
...
def load_machine(input_line: str) -> Machine:
    ...
    buttons = tuple(frozenset(int(num) for num in button.strip("() ").split(",")) for button in buttons_string.split())
...
def build_equation(solver: z3.Optimize, buttons: tuple[Button, ...], button_vars: list[z3.ArithRef], joltage: int, joltage_num: int):
    button_vars_that_matter = []
    for i, button in enumerate(buttons):
        if joltage_num in button:
            button_vars_that_matter.append(button_vars[i])
    print(f"{joltage} == sum({button_vars_that_matter})")
    solver.add(joltage == sum(button_vars_that_matter))


def solve_joltage_using_linalg(machine: Machine) -> int:
    buttons = machine[1]
    ...
    for joltage_num, joltage in enumerate(machine[2]):
        build_equation(solver, buttons, button_vars, joltage, joltage_num)

    solver.minimize(sum(button_vars))
    solver.check()
    model = solver.model()
    values = [model[var].as_long() for var in button_vars]
    print(f"{values=}")
    return sum(values)
...
def main():
    ...
    for i, mach in enumerate(machines):
        ...
        presses = solve_joltage_using_linalg(mach)
